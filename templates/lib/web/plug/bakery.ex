defmodule <%= app_module %>.Web.Plug.Bakery.Fetch do
  defmacro get_files() do
    static_files = Path.wildcard("client/dist/<%= app_module %>/*.{css,js,ico,html,png,jpg,gz,br}")
    ...
    static =
      ...
      |> Enum.map(fn file_to_send ->
        request_path =
          file_to_send
          |> String.replace_leading("client/dist/<%= app_module %>", "")
...
defmodule <%= app_module %>.Web.Plug.Bakery do
  alias <%= app_module %>.Web.Plug.Bakery.Fetch
